...
{
    public partial class DemandCustomerBackOrder : System.Web.UI.Page
    {
        ...
        protected void btnUpload_Click(object sender, EventArgs e)
        {
            ...
            if (FileUpload1.HasFile)
            {
                ...
                else
                {
                    try
                    {
                        string savepath = "C:\\Files\\DemandCustomerBackOrder_" + DateTime.Now.ToString("mmddyyhhmmss") + Extension;
                        objTextFileData.SavePath = "C:\\Files\\DemandCustomerBackOrder_" + DateTime.Now.ToString("mmddyyhhmmss") + Extension;
        ...
        protected void btnProcess_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                objTestBusiness.DeleteDemand_Customer_BackOrder(((UserLoginInfo)Session["UserLoginInfo"]).Lean_App);
                while (!(sReader.EndOfStream))
                {
                    ...
                    if (stringRead.StartsWith("|") & !stringRead.StartsWith("| Sales Doc") & !stringRead.StartsWith("|*") & !stringRead.StartsWith("|    "))
                    {
                        ...
                        bool result = objTestBusiness.CreateDemand_Customer_BackOrder(idLocalizationMaterial, idLocalization, idMaterial, sales_Doc, item, SLNo, Mat_Date, Doc_Ca, Quantity, Upload_User, Upload_Date, Lean_Application);
                        if (result == true)
                        {
                            counter++;
