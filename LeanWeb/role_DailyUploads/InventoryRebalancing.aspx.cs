...
{
    public partial class InventoryRebalancing : System.Web.UI.Page
    {
        int row_count = 0;
        ...
        //Sends the whole data to the business layer for database insertion
        protected void Bulk_Insert()
        {
            ...
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    objTestBusiness.DeleteInventoryRebalancing(Lean_App);
                    bool result = objTestBusiness.Insert_bulkInsertto_Inventory_Rebalancing(dt);
                    if (result == true)
                    {
        ...
        protected void gvInventoryRebalancing_PageIndexChanging(object sender, GridViewPageEventArgs e)
        {
            try
            {
                gvInventoryRebalancing.PageIndex = e.NewPageIndex;
                gvInventoryRebalancing.DataSource = FillDataTable(((ExcelData)Session["ExcelData"]).SavePath, ((ExcelData)Session["ExcelData"]).Extension);
                gvInventoryRebalancing.DataBind();
            }
            catch (Exception ex)
            {
                //syelamanchal--Logging--start
        ...
        protected void btnProcess_Click(object sender, EventArgs e)
        {
            ...
            {
                Bulk_Insert();
                gvInventoryRebalancing.Visible = false;
            }
            catch (Exception ex)
            {
                ex.Message.ToString();
            ...
            try
            {
                ...
                if (FileUpload1.HasFile)
                {
                    if (Extension != ".xls")
                    {
                        ...
                        {
                            savepath = "C:\\Files\\InventoryRebalancing_" + DateTime.Now.ToString("mmddyyhhmmss") + Extension;
                            objExcelData.SavePath = "C:\\Files\\InventoryRebalancing_" + DateTime.Now.ToString("mmddyyhhmmss") + Extension;
                            ...
                            if (dt.Rows.Count > 0)
                            {
                                ...
                                gvInventoryRebalancing.DataSource = dt;
                                gvInventoryRebalancing.DataBind();
                                gvInventoryRebalancing.Visible = true;
